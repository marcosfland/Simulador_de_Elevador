...
void entrar_elevador(Elevador *elevador) {
    if (elevador->ocupacao < elevador->capacidade) {
        elevador->ocupacao++;
        printf("Uma pessoa entrou no elevador. Ocupação atual: %d\n", elevador->ocupacao);
    } else {
        printf("O elevador está cheio. Não é possível entrar.\n");
    }
}
...
void sair_elevador(Elevador *elevador) {
    if (elevador->ocupacao > 0) {
        elevador->ocupacao--;
        printf("Uma pessoa saiu do elevador. Ocupação atual: %d\n", elevador->ocupacao);
    } else {
        printf("O elevador está vazio. Não é possível sair.\n");
    }
}
...
void mover_elevador(Elevador *elevador, int andar_destino) {
    if (andar_destino >= 1 && andar_destino <= NUM_ANDARES) {
        ...
        printf("Andar inválido. O elevador não se moveu.\n");
    }
}
